...
Start your own MongoDB.
Start the engine (use VM parameter `-Djboss.bind.address=0.0.0.0`, so that
cypress can access it).
On MacOS under Settings -> General -> Airdrop & Handoff disable AirPlay receiver
(collision on port 7000). Start the frontend.

`npm run cypress:open` will open the Cypress GUI. You can then run the tests from there.
...
To setup the user in Playwright tests you need to add the email of the user you will use as ADMIN user in the your instance of engine (in `defaults-dev.properties`). The tests loads the from environment variables. Create `.env` and add the email and user password into `.env` file
...
To prevent failing tests the user should not be already stored in the database and must not be stored in Auth0.

To run Playwright tests use `npm run playwright:run` command.

To open the HTML report after the testing use `npx playwright show-report` command.
