...
export function videosReducer(state: VideosState = initialVideosState, action: VideosAction.All): VideosState {
  switch (action.type) {
    ...
    case VideosActionType.SET_VIDEOS:
      return videosAdapter.addAll(action.payload.videos, videosAdapter.removeAll(state));
    case VideosActionType.REGISTER_VIDEO:
      return videosAdapter.upsertOne(action.payload, state);
    default:
      return state;
  }
}
