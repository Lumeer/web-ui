...
import {CollectionAttributePair} from './collection-attribute-pair';
...
export class Packer {
  ...
  public pack(documents: DocumentModel[]): CollectionAttributePair[] {
    return documents
  ...
  private packDocument(document: DocumentModel): Pair[] {
  ...
  private reducePairs(result: Pair[], pair: Pair) {
  ...
  private equal(pairA: Pair, pairB: Pair): boolean {
    return pairA[0] === pairB[0] &&
      pairA[1] === pairB[1];
  }

  private pairToCollectionAttributePair(pair: Pair): CollectionAttributePair {
    return {
      collection: pair[0],
      attributeId: pair[1]
    };
