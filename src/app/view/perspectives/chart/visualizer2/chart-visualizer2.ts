...
import {DataChange, PlotMaker2} from './plot-maker/plot-maker2';
import {createPlotMakerByType} from './plot-maker/plot-maker-util2';
import {DraggablePlotMaker2} from './plot-maker/draggable-plot-maker2';

export class ChartVisualizer2 {
  private data: Data[] = [];
  ...
  public createChartAndVisualize() {
    ...
    this.chartElement.nativeElement.on(
      'plotly_relayout',
      () => this.plotMaker instanceof DraggablePlotMaker2 && (this.plotMaker as DraggablePlotMaker2).onRelayout()
    );
  }
  ...
  private refreshDrag() {
    if (this.writable) {
      this.plotMaker instanceof DraggablePlotMaker2 && (this.plotMaker as DraggablePlotMaker2).initDrag();
    } else {
      this.plotMaker instanceof DraggablePlotMaker2 && (this.plotMaker as DraggablePlotMaker2).destroyDrag();
    }
  }
  ...
  public enableWrite() {
    this.writable = true;
    this.plotMaker &&
      this.plotMaker instanceof DraggablePlotMaker2 &&
      (this.plotMaker as DraggablePlotMaker2).setDragEnabled(true);
  }
  ...
  public disableWrite() {
    ...
    this.plotMaker &&
      this.plotMaker instanceof DraggablePlotMaker2 &&
      (this.plotMaker as DraggablePlotMaker2).setDragEnabled(false);
  }
}
