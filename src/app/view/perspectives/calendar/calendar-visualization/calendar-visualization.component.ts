...
export class CalendarVisualizationComponent implements OnChanges {
  ...
  private visualize() {
    ...
    this.allConfigs.forEach(config => {
      if (
        ...
        this.documents.filter(doc => doc.collectionId === config.id).forEach( document => {

          if (CalendarVisualizationComponent.isValidDate(document.data[config.barsProperties[CalendarBarPropertyRequired.START_DATE].attributeId])
            || CalendarVisualizationComponent.isValidDate(document.data[config.barsProperties[CalendarBarPropertyRequired.END_DATE].attributeId])) {
  ...
  private createEvent (document, title, start, end, config) {
    ...
    if (titleFromDocument !== '')
      ...
      //optional
      if (config.barsProperties[CalendarBarPropertyOptional.START_TIME] &&
        ...
        if (CalendarVisualizationComponent.isValidTime(startTime)
          ...
          const time = CalendarVisualizationComponent.createTime(startTime);
        ...
        if (CalendarVisualizationComponent.isValidTime(endTime)
          ...
          const time = CalendarVisualizationComponent.createTime(endTime);
          end.setHours(time[0]);
      ...
      return {
        ...
        color: CalendarVisualizationComponent.getColor(true, this.collections.find(collection => collection.id === config.id).color),
        allDay: false,
        draggable: true,
        resizable: {
          beforeStart: true,
  ...
  private static isValidTime(timeString) {
    if (timeString !== undefined) {
      ...
      return date.getHours().toString() === bits[0] && date.getMinutes().toString() === bits[1];
    }
    else return "0:00";
  }
  ...
  public decomposeEvent(event){
    ...
    originalDocument.data[configOfCollection.barsProperties[CalendarBarPropertyRequired.NAME].attributeId] = event.title;
    originalDocument.data[configOfCollection.barsProperties[CalendarBarPropertyRequired.START_DATE].attributeId] = CalendarVisualizationComponent.dateToString(event.start);
    originalDocument.data[configOfCollection.barsProperties[CalendarBarPropertyRequired.END_DATE].attributeId] = CalendarVisualizationComponent.dateToString(event.end);
