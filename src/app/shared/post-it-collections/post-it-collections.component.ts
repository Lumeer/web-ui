...
import {ProjectGetter} from './project-getter';
...
export class PostItCollectionsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private subscribeOnNavigation() {
    this.projectGetter = new ProjectGetter(this.store);
    ...
    this.navigationSubscription = this.store.select(selectNavigation).pipe(
      ...
      this.projectGetter.getProject(this.workspace.organizationCode, this.workspace.projectCode);
    });
  }
  ...
  public hasCreateRights(): boolean {
    return this.projectGetter.project && this.hasRole(this.projectGetter.project, Role.Write);
  }
