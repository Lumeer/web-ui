...
export class GetHierarchySiblingsBlocklyComponent extends BlocklyComponent {
  private tooltip: string;

  public constructor(public blocklyUtils: BlocklyUtils) {
    super(blocklyUtils);

    this.tooltip = $localize`:@@blockly.tooltip.getHierarchySiblingsDocumentsBlock:Gets the sibling records in hierarchy. Returns an empty array if there aren't any.`;
  }
  ...
  public registerBlock(workspace: any): void {
    ...
    Blockly.Blocks[BlocklyUtils.GET_HIERARCHY_SIBLINGS] = {
      init: function () {
        this.jsonInit({
    ...
    Blockly.JavaScript[BlocklyUtils.GET_HIERARCHY_SIBLINGS] = function (block) {
      const value_document = Blockly.JavaScript.valueToCode(block, 'DOCUMENT', Blockly.JavaScript.ORDER_ATOMIC) || null;

      const code = this_.blocklyUtils.getLumeerVariable() + `.getHierarchySiblings(${value_document})`;
  ...
  public getDocumentVariablesXml(workspace: any): string {
    return '<xml><block type="' + BlocklyUtils.GET_HIERARCHY_SIBLINGS + '"></block></xml>';
  }

  private checkWorkspaceChange(workspace, changeEvent, block) {
    if (isNotNullOrUndefined(block) && block.type === BlocklyUtils.GET_HIERARCHY_SIBLINGS) {
